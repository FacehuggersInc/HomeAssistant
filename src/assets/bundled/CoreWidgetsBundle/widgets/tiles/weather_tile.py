...
# the background is doing most of that work - the icon only has to confirm it.
...
class WeatherTile(Tile):
    ...
    A worked example of size variants: the same tile is a glanceable icon at
    one cell, gains an hourly strip at 2x3, and becomes a full readout at 3x3
    and above.
    """
    ...
    def __init__(self, client: "Client", grid_w: int = 2, grid_h: int = 2):
        self._is_day    = True
        self._icon_name = "mdi.weather-cloudy"
        self._current    = None
        self._hourly     = []
    ...
    def paintEvent(self, event) -> None:
        ...
        if self._is_day:
            gradient.setColorAt(0.0, DAY_TOP)
            gradient.setColorAt(1.0, DAY_BOTTOM)
        else:
            gradient.setColorAt(0.0, NIGHT_TOP)
            gradient.setColorAt(1.0, NIGHT_BOTTOM)
            ...
            def apply():
                try:
                    ...
                    if current:
                        ...
                        self._icon_name = api.get_icon(current)
                    if hourly:
                        self._hourly = hourly
                    self._repaint_from_cache()
                except RuntimeError:
    ...
    def _apply_glance(self) -> None:
        ...
        scale = getattr(self, "_glance_scale", 1.0)
        size = max(26, int(min(self.width(), self.height()) // 3 * scale))
        try:
            self.glyph.setPixmap(
                icon(self._icon_name, color="#ffffff").pixmap(size, size))
        except Exception:
            self.glyph.setText("")
    ...
    def _apply_details(self) -> None:
        ...
        rows = [
            ...
            ("Precipitation", self._inches("precipitation")),
            ("Rain",         self._inches("rain")),
            ("Showers",      self._inches("showers")),
            ("Snowfall",     self._inches("snowfall")),
            ("Wind",         self._wind(api)),
